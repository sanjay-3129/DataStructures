...
public class CircularDoubleLinkedList<T> {
	...
	public void add(T value) {
		...
		if (size == 0 && !existsLinkedList()) { // set in head
			...
			node.setNext(node);  // for CDLL
			node.setPrev(node);  // for CDLL
			head = node;
			tail = node;
		} else { // size != 0, then add at last/tail
			node.setNext(head);  // for CDLL
			node.setPrev(tail);
			head.setPrev(node);
			tail.setNext(node);
			tail = node;
	...
	public void circularTraverseLinkedList() {
		if (existsLinkedList()) {
			DoubleNode<T> tempNode = head;
	...
	// print all elements in linkedlist
	public void circularReverseLinkedList() {
		if (existsLinkedList()) {
			DoubleNode<T> tempNode = tail;
			for (int i = 0; i < getSize()*2; i++) {
				System.out.print(tempNode.getData());
