...
public class CircularSingleLinkedList<T> {
	...
	public void add(T value) {
		...
		if(size == 0 && !existsLinkedList()) {  // set in head
			...
			node.setNext(node);   // for circularSLL
			head = node;
			tail = node;
		} else {    // size != 0, then add at last/tail
			node.setNext(head);  // for circularSLL
			tail.setNext(node);
			tail = node;
		}
	...
	public void add(int index, T value) {
		...
		if(!isLL && (index>0 || index<0)) {
			...
			node.setNext(node);  // for CSLL: null -> node
			head = node;
			tail = node;
			...
			if(index == 0) {   // insert at first position
				...
				// update last elements next
				tail.setNext(head);  // for CSLL
			} 
			else if(index >= size) {  // insert at last
				node.setNext(head);  // for CSLL: null -> node
				tail.setNext(node);
				tail = node;
			}
	...
	// print all elements in linkedlist twice to check circular connectivity
		public void traverseLinkedListTwice() {
			if(existsLinkedList()) {
				SingleNode<T> tempNode = head;
	...
	public void deleteNode() {
		if(!existsLinkedList()) {
			...
			tempNode.setNext(head);   // for CSLL -> null to head 
			tail = tempNode;
			setSize(getSize()-1);
		}
	}
	...
	public void deleteNode(int index) {
		if(!existsLinkedList()) {
			...
			tempNode.setNext(head);    // for CSLL
			tail = tempNode;
			setSize(getSize()-1);
		}
